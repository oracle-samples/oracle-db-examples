...
var samp number
--
-- Assuming upper-case names only
--
exec select decode(upper('&1'),'USER',user,null, user, upper('&1')) into :ownname from dual;
exec select upper('&2') into :tabname from dual;
exec select '&3' into :samp from dual;
exec select decode(nvl(upper('&4'),'N'),'N','N','Y') into :create_now from dual;
