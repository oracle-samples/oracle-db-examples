...
declare
  ...
  cursor partition_list is
     select partition_name
     from   dba_tab_partitions
     where  table_name  = p_tabname
     and    table_owner = p_ownname
  ...
  cursor current_cols is
     ...
     from   dba_part_col_statistics s,
            dba_tab_columns c
     where  s.table_name = p_tabname
     and    s.owner      = p_ownname
     and    s.table_name  (+) = c.table_name
     and    s.owner       (+) = c.owner
     and    s.column_name (+) = c.column_name
     and    s.partition_name = v_partname
     order by s.partition_name,c.column_id;

  cursor distinct_tim is
     select distinct savtime,SAMPLE_DISTCNT
     from   sys.WRI$_OPTSTAT_HISTHEAD_HISTORY
     where  intcol# = v_colnum
     and    obj# = (select object_id from dba_objects 
                    where object_name = p_tabname and subobject_name = v_partname and owner=p_ownname and object_type = 'TABLE PARTITION')
     order by savtime;
...
begin
   ...
   loop
      v_partname := pl.partition_name;
      dbms_output.put_line('=========================================================');
      dbms_output.put_line('Partition Name : '||v_partname);
      dbms_output.put_line('=========================================================');
