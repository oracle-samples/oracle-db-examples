...
exec ctx_ddl.drop_preference('multi_lex')
exec ctx_ddl.drop_preference('english_lex')
exec ctx_ddl.drop_preference('korean_lex')

begin
    ctx_ddl.create_preference('english_lex','BASIC_LEXER');
    ctx_ddl.create_preference('korean_lex','KOREAN_MORPH_LEXER');
end;
/

begin
    ctx_ddl.create_preference('multi_lex', 'multi_lexer');
    ctx_ddl.add_sub_lexer    ('multi_lex','DEFAULT','english_lex');
    ctx_ddl.add_sub_lexer    ('multi_lex','KOREAN','korean_lex', 'kor');
end;
/

create index multiindex on multitext (text) 
indextype is ctxsys.context
parameters ('lexer multi_lex language column language_ident');
