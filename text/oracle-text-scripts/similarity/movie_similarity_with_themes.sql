...
begin
    ...
    for c in (  
        select 
            ...
            title     
        from movie_similarity 
        where contains(title, contains_clause, 1) > 0 
...
create index movie_summary_index on movie_similarity(summary) indextype is ctxsys.context;

create or replace function get_themes(p_rowid rowid) return string is
  themearr ctx_doc.theme_tab;
  outstr   varchar2(4000);
begin 
  ctx_doc.themes(
      index_name => 'movie_summary_index', 
      textkey    => p_rowid, 
      restab     => themearr,
      num_themes => 10);
  for t in themearr.first .. themearr.last loop
    outstr := outstr || themearr(t).theme;
    if t != themearr.last then 
       outstr := outstr || ', ';
    end if;
  end loop;
  return outstr;
...
-- select rowid,title from movietab where title like 'Rocky%';

alter table movie_similarity add (themes varchar2(2000));

update movie_similarity set themes = get_themes(rowid);
