...
create package body ctx_diag as
  ...
  c_version varchar2(64) := 'Feb 19, 2020';
  ...
  begin
    ...
    -- prepare $R
    if p_resume then                   -- find first DocID to resume processing
      begin
        execute immediate
          'select row_no, length(data) '||
            'from '||p_rtab||' '||
           'where row_no = (select max(row_no) '||
                             'from '||p_rtab||')'
        into v_row, v_len;
        dbms_output.put_line('v_row = '||v_row||', v_len = '||v_len);

        v_first := c_max*v_row + v_len/14;             -- redo last valid DocID
	dbms_output.put_line('v_first = '||v_first);
	if v_len = c_max*14 then
	  v_len := 0;
          v_prev := v_row;
        end if;
        dbms_output.put_line('now v_row = '||v_row||', v_len = '||v_len||', v_prev = '||v_prev);
      exception
        when no_data_found then
          v_first := 1;                                          -- $R is empty
          v_len   := 0;                                    -- nothing to resume
      end;
    else                                                         -- truncate $R
      execute immediate
    ...
    -- read $K
    dbms_output.put_line('fetching rowids for docid > '||v_first);
    ...
    loop
      ...
      -- process the batch
      dbms_output.put_line('processing batch from 1 to '||v_tab.count||' docids '||v_tab(1).docid||' to '||v_tab(v_tab.count).docid);
      for i in 1..v_tab.count loop
        v_row := trunc((v_tab(i).docid - 1) / c_max);

        -- init lob locator
	dbms_output.put_line('init lob : r_row '||v_row||', v_prev = '||v_prev);
        while v_row > v_prev loop
          ...
          -- create new BLOB
          dbms_output.put_line('testing v_len : '||v_len);
          if v_len = 0 then
            dbms_output.put_line('v_len is zero - new row needed');
            v_off  := 1;
            v_prev := v_prev + 1;

            dbms_output.put_line('inserting new row '||v_prev);
            execute immediate 
          ...
          dbms_output.put_line('selecting data from rtab where row_no = '||v_prev);
          execute immediate
            'select data '||
            'from  '||p_rtab||' '||
            'where  row_no = :1 '||
	  ...
	  dbms_output.put_line('select done - next DOCID is '||to_char(v_tab(i).docid));
        end loop;

        -- write rowid
	-- dbms_output.put_line('write rowid');
      ...
      --dbms_output.put_line('restart select');
      execute immediate 
        'select data '||
        'from  '||p_rtab||' '||
        'where  row_no = :1 '||
        'for update'
      into v_loc using v_prev;
      --dbms_output.put_line('restart select done');
    end loop;
    close v_cur;
--  exception
--    when others then
