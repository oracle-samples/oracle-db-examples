--
-- This package is for analyzing problems with the Oracle Text index $K and $R
-- tables.
--
-- The package should be installed as the CTXSYS user then called by the index
-- owner. To install as CTXSYS, you should run these commands in SQL*Plus:
--
--   connect / as sysba   (or SYS/password as sysdba)
--   alter session set current_schema=ctxsys
--   @ctxdiag
--
-- The various procedures are called with table names, such as the $R table
-- name. In generally, this will be
--    DR$<indename>$R for unpartitioned indexes, and
--    DR#<indexname><partitionnumber>$R for a partitioned index
-- However, these names may be different for long index names, or very large
-- numbers of partitions. The user should check carefully the names of tables
-- in the current schema before proceeding.
--

-- The following drops are expected to give errors if the package has not been
...
create package ctx_diag authid current_user as
  ...
  -- write one $R row from $K
  procedure one_row(
    p_ktab      varchar2,
    p_rtab      varchar2,
    p_row       integer,                                           -- $R row_no
    p_max       integer,                 -- MAXimum number of DocIDs per $R row
    p_next      integer);                             -- next DocID to allocate
  ...
  -- clear one DocID in $R
  procedure clear_r(
    p_rtab   varchar2,
    p_docid  number);

  -- wrap large $R rows to small rows 
  procedure wrap_r(
    p_rtab   varchar2,
    p_size   integer default 70000);
...
create package body ctx_diag as
  ...
  c_version varchar2(64) := 'Nov 05, 2020';
  ...
  -- finds maximum non-empty DocID in $R and clears empty rows at the tail
  --   returns 0 when $R is empty or all slots are empty
  function backtrack(
    p_rtab   varchar2,
    p_max    integer)
  return integer
  is
    v_cur       sys_refcursor;                                     -- $R cursor
    v_row       integer := -1;                             -- current $R row_no
    v_loc       blob;                                         -- $R data column
    v_len       integer;                                         -- BLOB LENgth
    v_buf       raw(14);                          -- $R compressed rowid BUFfer
    v_hex       varchar2(28);                        -- rowid buffer HEX string
    v_doc       integer := 0;                                          -- DOCid
    v_off       integer;                                         -- BLOB OFFset
    v_size      integer := 14;                                 -- $R rowid SIZE
  begin
    -- find max DocID
    open v_cur for 'select row_no, data from '||p_rtab||' order by 1 desc';
    loop
      fetch v_cur into v_row, v_loc;
      exit when v_cur%notfound;
      ...
      while v_off >= 1 loop
        ...
        if v_hex != '0000000000000000000000000000' then          -- not deleted
          v_doc := p_max*v_row + (v_off + 14 - 1)/14;
          exit;
        end if;
    ...
    -- treat all deleted as empty table
    if v_doc = 0 then
      v_row := -1;
    end if;
    ...
    -- clear empty rows
    execute immediate
      'delete from '||p_rtab||' where row_no > :1'
    using v_row;
    commit;
  ...
  procedure one_row(
    ...
    p_next      integer)                              -- next DocID to allocate
  is
    v_first     integer;                     -- FIRST DocID to start processing
    v_last      integer;                      -- LAST DocID to start processing
    v_curr      integer := 0;                 -- CURRent maximum DocID in a row
  ...
  begin
    -- compute $R range
    v_first   := p_row * p_max + 1;
    v_last    := (p_row + 1) * p_max;
    ...
    loop
      ...
      execute immediate
        ...
        'where  row_no = :1 '||
        'for update'
      into v_loc using p_row;

      -- process the batch
      for i in 1..v_tab.count loop
        v_off := mod(v_tab(i).docid - 1, p_max) * 14 + 1;
        write_rowid(v_loc, v_off, v_tab(i).textkey);
        v_curr := v_tab(i).docid;
      end loop;
    ...
    -- check is we need to fill any gaps
    v_off := 0;
    if v_last < p_next - 1 then                                 -- not last row
      if v_curr < v_last then                                  -- fill the tail
        v_off := (p_max - 1) * 14 + 1;                       -- fill to the end
      end if;
    else                                                            -- last row
      if v_curr < p_next - 1 then              -- there are DELETEs at the tail
        v_off := mod(p_next - 2, p_max) * 14 + 1;           -- fill to the last
      end if;
    end if;

    -- fill $R row with zeros
    if v_off > 0 then
      -- init lob locator
      execute immediate
        'select data '||
  ...
  begin
    ...
    -- prepare $R
    if v_resume then                   -- find first DocID to resume processing
      v_first := backtrack(p_rtab, p_max);
      if v_first = 0 then                                        -- $R is empty
        v_first  := 1;
        v_resume := FALSE;
      end if;
    else                                                         -- truncate $R
  ...
  begin
    ...
    -- nothing to wrap
    if v_maxlen <= p_size*14 then
      return;
    end if;

    -- get large row locator
    execute immediate
     'select data '||
       'from '||p_rtab||' '||
      'where row_no = 0'
    ...
    if v_maxrow = 0 then
      ...
      execute immediate
       'select length(data) '||
         'from '||p_rtab||' '||
        'where row_no = :row_no'
      into v_len using v_row;

      -- skip full row
      if v_len = p_size*14 then
        v_row := v_row + 1;
        v_len := 0;
      end if;
    end if;
    ...
    -- wrap
    loop
      v_soff := v_len + v_row*p_size*14 + 1;
      v_doff := v_len + 1;
      v_amt  := p_size*14 - v_len;
      if v_soff > v_maxlen then
        exit;
      elsif v_soff + v_amt > v_maxlen + 1 then
        v_amt := v_maxlen + 1 - v_soff;
      end if;

      -- start new row
      if v_row > v_maxrow then
        execute immediate 
         'insert into '||p_rtab||' (row_no, data) '||
         'values (:1, empty_blob())'
        using v_row;
      ...
      execute immediate
       'select data '||
        ...
        'where row_no = :row_no '||
          'for update'
      into v_dst using v_row;

      -- copy
      dbms_lob.copy(v_dst, v_src, v_amt, v_doff, v_soff);
      commit;
