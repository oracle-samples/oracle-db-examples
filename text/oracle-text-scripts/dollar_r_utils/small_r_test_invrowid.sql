...
select id from mytable where rowid in (select textkey from dr$myindex$k where docid = 80000);

exec dollarr_utils2.invalid_rowid(80000);

select * from mytable where contains(text, 'hello') > 0 and id = &enterid;
