-- This package converts a conventional non-small-r index into a ** 5K ** 
...
--    0.93     roger.ford  20181213  Modified to create "very small" 5K $R rows
--                                   Should work on full-size $R or 35K $R
--
...
alter session set plsql_warnings='ENABLE:ALL', 'DISABLE:6025';
...
begin
   ...
   -- SMALL_R_ROW $R table will have 5K docs per row
   -- but check for 35K and 70K settings
   -- so insert 71K docs:
   ...
   if row_count <= 1 then
      ...
      cleanup;
      raise_application_error(-20000, 'SMALL_R_ROW setting not working on this system - large $R rows created');
   end if;
   ...
   if row_count = 3 then
      dbms_output.put_line('only 3 $R rows created. Cleanup then raise error');
      cleanup;
      raise_application_error(-20000, 'SMALL_R_ROW uses 35K row size. Install one-off patch for very small (5K) row size before attempting conversion');
   end if;

   if row_count = floor(71000/newRowSize)+1 then
      dbms_output.put_line(to_char(row_count)||' rows created, $R uses correct row size. Cleaning up temp table and index');
      cleanup;
      return 'OK';
   end if;

   -- otherwise ...
   dbms_output.put_line('Unexpected ' || row_count || ' rows created. Cleanup then raise error');
   cleanup;
   raise_application_error(-20000, 'Unexpected row count (' || row_count || ') found in $R table. Unable to determine state of system. Aborting.');
...
chunkSize   number := (newRowSize*14)/1000;
   ...
   while not part_cursor%NOTFOUND loop
      ...
      while not ftch_cursor%NOTFOUND loop
         ...
         while (amount = chunkSize) loop
            ...
            -- at this point we've read up to chunkSize bytes of data which needs to be
