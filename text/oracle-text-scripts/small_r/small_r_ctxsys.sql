...
--
--   This version has been split into modules to be called by the SYS user, and modules
--   to be called by the index owner

--      connect / as sysdba
--      exec small_r_convert.convert_index_metadata('<indexowner>', '<indexname>')
--      connect fusion/fusionpassword
--      exec sys.small_r_convert.convert_my_index('<indexname>')
--      connect / as sysdba
--      exec small_r_convert.cleanup
...
--      execute sys.small_r_convert.convert_index_metadata( ownername, indexname)
--        eg.
--      execute sys.small_r_convert.convert_index_metadata( 'roger', 'myindex' )
--   THEN connect as the index owner, and run:
--      execute sys.small_r_convert.convert_my_index ( indexname )
--        eg.
--      connect roger/roger
--      execute sys.small_r_convert.convert_my_index ( 'myindex' )
--   FINALLY connect as SYS again and run the cleanup procedure
--        eg.
--      connect / as sysdba
--      execute sys.small_r_convert.cleanup
--
--   *********************************************************************************
...
--    1.00     roger.ford  20190123  Split into parts so can be run in a Database VAULT env.
...
create or replace package small_r_convert 
   ...
   procedure convert_index_metadata (owner varchar2, indexname varchar2);

   procedure convert_my_index (indexname varchar2);
...
procedure convert_index_metadata
   ( owner     varchar2, 
     indexname varchar2) 
   as
...
--   end if;
   ...
   -- first we need to provide public access to the metadata where the $R table clause is stored, otherwise
   -- update_my_index can't work.

   ssql := 'create or replace view get_r_table_clause as (
select ixv_value , idx_name 
 from 
 ...
 where ixv_idx_id = idx_id
   ...
   ssql := 'grant select on get_r_table_clause to public';
   execute immediate ssql;
...
end convert_index_metadata;

procedure convert_my_index
   ( indexname varchar2) 
   as
...
begin
   ...
   begin
      select idx_id into idxid from ctxsys.ctx_user_indexes where idx_name = vindexname;
   exception when no_data_found then
      raise_application_error(-20000, 'index '||vindexname||' not found');
   end;
...
select ixv_value 
 from 
   ctxsys.get_r_table_clause
   where idx_name   = :indexname
';

   execute immediate ssql into r_tab_cls using vindexname;

   -- index exists. Is it partitioned?

   select count(*) into partcnt from ctxsys.ctx_user_index_partitions 
     where ixp_index_name = vindexname;
...
--   ssql := 'select ixv_value from ctxsys.ctx_user_index_values where ixv_index_owner = :owner and ixv_index_name = :indexname and ixv_attribute = ''SMALL_R_ROW''';
--   begin
--      execute immediate ssql into ixvvalue using vindexname;
--   exception when NO_DATA_FOUND then
--      ixvvalue := 'NO';
--   end;
...
--   end if;
   ...
   ssql := 'select count(*) from user_tables where table_name like :oldtable';

   if partcnt > 0 then
      execute immediate ssql into row_count using 'DR#' || vindexname || '%' || '$RO';
      if row_count > 0 then
         raise_application_error(-20000, 'Previous $R table '||'DR#'|| vindexname || 'nnnn$RO found. Drop before proceeding if not needed');
      end if;
   else
      execute immediate ssql into row_count using 'DR$' || vindexname || '$RO';
      if row_count > 0 then
         raise_application_error(-20000, 'Previous $R table '||'DR$' || vindexname || '$RO' || ' found. Drop before proceeding if not needed');
      end if;
   end if;
   ...
   if partcnt > 0 then
      ssql := 'select ixp_id from ctxsys.ctx_user_index_partitions 
              where ixp_index_owner = user and ixp_index_name = :indexname';
   else
      ssql := 'select -1 from dual where 1=1 or user = :1';
   end if;

   open part_cursor for ssql using vindexname;
   ...
   while not part_cursor%NOTFOUND loop
      ...
      if partid = -1 then 
         new_table := 'DR$' || vindexname || '$RN';
         old_table := 'DR$' || vindexname || '$R';
         old_table_no_schema  := 'DR$' || vindexname || '$R';
         constraint          := 'DRC$' || vindexname || '$R';
      else
         new_table := 'DR#' || vindexname || lpad(partid, 4, '0') || '$RN';
         old_table := 'DR#' || vindexname || lpad(partid, 4, '0') || '$R';
      ...
      while not ftch_cursor%NOTFOUND loop
         ...
         while (amount = 490) loop
            ...
            -- at this point we've read up to 490 bytes of data which needs to be
...
end convert_my_index;

procedure cleanup is
   ssql varchar2(255);
begin
   ssql := 'drop view get_r_table_clause';
   dbms_output.put_line('Execute: '||ssql);
   execute immediate(ssql);
   ssql := 'alter system flush shared_pool';
   dbms_output.put_line('Execute: '||ssql);
