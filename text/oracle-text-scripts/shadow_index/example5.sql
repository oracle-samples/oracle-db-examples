...
exec ctx_ddl.create_shadow_index('myindex', 'replace lexer mypref NOPOPULATE')

variable idxid number
exec select idx_id into :idxid from ctx_user_indexes where idx_name = 'MYINDEX'

exec ctx_ddl.populate_pending('RIO$'||:idxid)

exec ctx_ddl.sync_index('RIO$'||:idxid)
