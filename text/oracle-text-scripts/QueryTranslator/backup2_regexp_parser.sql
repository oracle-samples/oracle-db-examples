  ...
  while true loop
    ...
    if regexp_instr (match, '^' || lc || '"') > 0 then
       ...
       if instr (leadChars, substr (match, 1, 1)) > 0 then  
         p ('plussed match');
         match := regexp_replace (match, '[^[:space:][:alnum:]'||joinchars||']+', ' ', 2);
       else
         p ('nonplussed match');
         p ('looking for '||substr (match, 1, 1));
         match := regexp_replace (match, '[^[:space:][:alnum:]'||joinchars||']+', ' ');
       end if;
    ...
    if substr(match, length(match), 1) = userWild then
       p ('regexp_replace ('''||match||''', '''||ec|| '$' || ''', '''|| sysWild || ''')');
       p ('ec: '||ec);
       p (sysWild);
       p (match);
       match := regexp_replace (match, ec || '$', sysWild, 1, 1);
       p (match);
    else
       p ('not wild');
    end if;
