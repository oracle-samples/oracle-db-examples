...
--     or in translated form (b, c, d) WITHIN a1
...
begin
  ...
  for i in 1 .. global_stopList.last loop
    if lower(w) = lower(global_stopList(i)) then
      p('matched. "'||w||'" is a stopword');
      return false;
    end if;
  end loop;
  p('"'|| w || '" is not a stopword');
  return true;
end;
    ...
    if allList.count < 2 then
      ...
      for i in 1..allList.last loop
        for k in i+1..(allList.last) loop
          if i != k then
	    p('Words i='||allList(i)||' and k='||allList(k));
	    if ( notStopWord(allList(i)) and notStopWord(allList(k))) then 
              p('both non-stopword');
	      p('adding ' || conj ||global_nearOper||'(('||allList(i)||','||allList(k));
	      retStr := retStr || conj ||global_nearOper||'(('||allList(i)||','||allList(k)||'))';
              conj := ',';
	    else
	      p('at least one is a stopword');
            end if;
          end if;
        end loop;
      end loop;
