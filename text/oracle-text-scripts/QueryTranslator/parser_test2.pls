...
--  /* Version 0.99: replaced "-" not preceded by space with space */
--
--   p('QueryString before: '||queryString);
--
--   l_queryString := regexp_replace (queryString, '([^[:space:]])-', '\1 ');

  l_queryString := queryString;

  p('QueryString after : '||l_queryString);
  ...
  reParse (l_queryString, words, global_joinChars, userWild => global_wildCard);
    ...
    case firstChar
      ...
      when '-' then
        ...
        querySegments.negTerms(querySegments.negTerms.last()) := 
          substr(qry, rstart + 1, secEnd - rstart);
        p('Added negative term: ' || substr(qry, rstart + 1, secEnd - rstart));
      else
