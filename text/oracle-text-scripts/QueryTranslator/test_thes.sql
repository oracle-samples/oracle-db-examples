...
exec ctx_thes.create_thesaurus('default')
exec ctx_thes.create_relation('default', 'canine', 'SYN', 'dog')
exec ctx_thes.create_relation('default', 'canine', 'SYN', 'doggy')
...
select parser.progrelax('cat syn(canine)') "Query Text" from dual;
select score(1) score, text from avtest where contains (text, parser.progrelax(
'cat syn(canine)'
),1) > 0 order by score(1) desc;
