...
type subqListType is table of varchar2(4000) index by varchar2(64);
...
function OTSimpleSectionSearch (userStr varchar2, indexName varchar2 default '') return varchar2;
...
begin
  -- insert into debug_table values (debug_seq.nextval, instr);
  -- commit;
...
procedure regexp_clob_split
  (str in out clob, match out varchar2, pattern varchar2, preceed in out clob)
is
  rest    varchar2(32767) := str;
  newstart integer;
  reStart  integer;
...
-- Find the end of a parenthesised string. We get a string without the first
-- open paren, and have to return the full string before the final close paren
-- so for example if given foo (bar) ) (baz) we should return foo (bar)

procedure getParenStr (inputStr varchar2, content in out varchar2) is
  pos number;
  c varchar2(1);
  openCount number := 0;
  closeCount number := 0;
begin
  ...
  loop 
    exit when inputStr is null or closeCount = openCount or pos > length(inputStr);
    if substr(inputStr, pos, 1) = '(' then
      openCount := openCount + 1;
    elsif substr(inputStr, pos, 1) = ')' then
      closeCount := closeCount + 1;
    end if;
    pos := pos+1;
    --p('Open: '||openCount||' Closed: '||closeCount|| ' Pos:'||pos);
  end loop;

  --p('Exited loop');
  if closeCount = openCount then 
    --p('found closed paren');
    content := substr(inputStr, 1, pos-2);
  else -- mismatched parentheses: return rest of string
    -- todo : could set an error flag (third arg?) if required
    --p('mismatched paren');
    content := substr(inputStr, 1, pos-1);
  end if;

end; 

procedure sectionSplitter ( 
       query      in      varchar2,     -- query to process
       retList    in out  subqListType, -- list of "optional" section searches
       plusList   in out  subqListType, -- list of "must have" section searches
       nonSection in out  varchar2      -- remaining non-section search criteria
) is
   qry    varchar2(4000);  -- todo: shouldn't this be a CLOB?
   match  varchar2(4000);
   re     varchar2(64);
   preStr varchar2(4000);
...
begin
  ...
  loop
     ...
     regexp_split(qry, match, re, preStr); 
     if match is null or length(match) = 0 then
        nonSection := nonSection || separator || qry;
        exit;
     end if;

     sectionName := regexp_replace(match, ':\(', '');
     -- need to match balanced ending parentheses. Can't do this in regular expressions
     getParenStr(qry, match);
     -- delete start of query to that point
     qry := substr( qry, length(match)+2 );

     if substr( sectionName, 1, 1) = '+' then
       plusList( substr(sectionName, 2, 1) ) := match;
     else 
       retList(sectionName) := match;
     end if;

     if length(preStr) > 0 then
        nonSection := nonSection || separator || preStr;
        separator := ' ';
     end if;

  end loop;

  --p('nonSection: "'||nonSection||'"');
...
is 
  ...
  sectionQrys     subqListType;
  plusQrys        subqListType;
  i               integer;
  retStr          varchar2(4000) := '';

  conj            varchar2(5)    := ''; -- ' AND ', ' OR ' or null
  joinStr         varchar2(5)    := '';
  subq            varchar2(4000) := ''; -- subquery for a section
  rest            varchar2(4000) := ''; -- stuff that's not part of a section query
begin

  sectionSplitter( userStr, sectionQrys, plusQrys, rest );  -- to do - validate sections?

  -- process the plus (must have) sections first, if any

  if plusQrys.count > 0 then
    subq := plusQrys.first;
    p('plus subq 1 '||subq);

    retStr := retStr || '(';
    loop
      exit when subq is null;
      retStr := retStr || conj || '( ' || OTSearchString ( plusQrys(subq), indexName ) || ') WITHIN '||subq || ')';
      conj := ' AND ';
      subq := plusQrys.next(subq);
    end loop;
    retStr := retStr || '),';
  end if;
  ...
  conj := null;
  subq := sectionQrys.first;
  p('Subq 1 is '|| subq);
  loop
    exit when subq is null;
    --p('Subq 2 is '|| subq);
    retStr := retStr || conj || '( ' || OTSearchString ( sectionQrys(subq), indexName ) || ') WITHIN '||subq || ')';
    conj := ' AND ';
    subq := sectionQrys.next(subq);
  end loop;
  if plusQrys.count > 0 then
    retStr := retStr || ')';
  end if;

  rest := OTSearchString( rest );

  if length(rest) > 0 then
    retStr := retStr || conj || '(' || OTSearchString ( rest ) || ')';
  end if;
