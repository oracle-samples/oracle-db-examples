...
This is the main text of the document. In this example the metadata and text are in-line.
...
select count(*) from t where contains (c, 'main text AND value77 within field77 AND SDATA(sdata1 < 30)') > 0;
...
insert into t values ('<sdata2>99</sdata2>');
commit;

select count(*) from t where contains (c, 'sdata(sdata2 between 90 and 100)') > 0;
