...
--   https://github.com/oracle-samples/oracle-db-examples/blob/main/machine-learning/sql/26ai/dmsh.sql
...
-- (26ai Feature) Create an ESA model with dense projections, which is  
-- similar to a doc2vec approach, by specifying the ESAS_EMBEDDINGS 
-- parameter as ESAS_EMBEDDINGS_ENABLED.

-- (26ai Feature) Use the dense projection scoring results to create a  
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE ESA_text_sample_settings';
...
---------------------
-- CREATE A NEW MODEL
-- Note that the transform treats the column 'comments'
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('ESA_text_sample');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
-- 'TEXT(POLICY_NAME:DMDEMO_ESA_POLICY)(TOKEN_TYPE:STEM)');
  ...
  v_setlst(dbms_data_mining.algo_name) := dbms_data_mining.algo_explicit_semantic_analys;
  v_setlst(dbms_data_mining.prep_auto) := dbms_data_mining.prep_auto_on;
  v_setlst(dbms_data_mining.odms_text_policy_name) := 'DMDEMO_ESA_POLICY';
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE ESA_text_sample_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/

BEGIN
INSERT INTO ESA_text_sample_settings (setting_name, setting_value) VALUES
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('ESA_text_sample2');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('ESA_text_sample_dense');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
DECLARE
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE esa_dense_results';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
column value format 9.99999
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('CLUSTERING_EXAMPLE');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
-- BEGIN
--   EXECUTE IMMEDIATE 'DROP VIEW MINING_BUILD_TEXT_PARALLEL';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP VIEW MINING_TEST_TEXT_ONLY';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP VIEW ESA_DENSE_RESULTS_V';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /
