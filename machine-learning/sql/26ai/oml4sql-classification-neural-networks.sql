...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE nnc_sh_sample_class_wt';
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('NNC_SH_Clas_sample');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
  ...
  v_setlst(dbms_data_mining.clas_weights_table_name) := 'nnc_sh_sample_class_wt';
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE nnc_sh_sample_settings';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
BEGIN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE nnc_sh_sample_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/
