...
BEGIN
  v_setlst(dbms_data_mining.kmns_distance) := dbms_data_mining.kmns_euclidean;
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE km_sh_sample_settings';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
BEGIN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE km_sh_sample_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/
...
-- not reference a pre-built clustering model, but rather it segments
