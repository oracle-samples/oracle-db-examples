...
--   . TXT_SAMPLE_BUILD_TEXT_IDX - an Oracle Text index built by this
--     script on a private copy of MINING_BUILD_TEXT.
-- 
...
/
BEGIN EXECUTE IMMEDIATE 'DROP INDEX txt_sample_build_text_idx';
EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE txt_sample_build_text';
EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
-- Step 1. Copy the shared sample text view to a private table.
--         An Oracle Text CONTEXT index requires a table, not a view.

CREATE TABLE txt_sample_build_text AS
SELECT cust_id, comments
FROM mining_build_text;

-- Step 2. Create the Text index used for term extraction.

CREATE INDEX txt_sample_build_text_idx
ON txt_sample_build_text(comments)
INDEXTYPE IS ctxsys.context;

-- Given the presence of these two schema objects, you can now
-- proceed to extract terms FROM text documents in TXT_SAMPLE_BUILD_TEXT
...
--         The parameters are:
--         'TXT_SAMPLE_BUILD_TEXT_IDX', -- index_name - Text index name
...
-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_TEXT_CAT';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP INDEX TXT_SAMPLE_BUILD_TEXT_IDX';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_BUILD_TEXT';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_PREF_TERMS';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_TERM_OUT';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_TERMS';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE NONTXT_ATTRIB';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP TABLE TXT_SAMPLE_DATA_INPUT';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /
