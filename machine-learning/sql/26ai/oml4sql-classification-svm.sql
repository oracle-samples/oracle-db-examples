...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE svmc_sh_sample_class_wt';
...
-- higher penalty than an error in the second class. Weights of (0.5, 0.5)
...
-- SVM chooses a kernel type automatically. This choice can be overridden
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('SVMC_SH_Clas_sample');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
  ...
  v_setlst(dbms_data_mining.clas_weights_table_name) := 'svmc_sh_sample_class_wt';
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE svmc_sh_sample_settings';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
BEGIN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE svmc_sh_sample_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/
...
-- This query demonstrates how to generate a confusion matrix using the
