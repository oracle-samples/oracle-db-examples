...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE rf_sh_sample_cost';
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('RF_SH_Clas_Sample');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
  v_setlst(dbms_data_mining.algo_name) := dbms_data_mining.algo_random_forest;
  v_setlst(dbms_data_mining.clas_cost_table_name) := 'rf_sh_sample_cost';
  v_setlst(dbms_data_mining.rfor_num_trees) := '25';
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE rf_sh_sample_settings';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
BEGIN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE rf_sh_sample_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/

BEGIN
INSERT INTO rf_sh_sample_settings (setting_name, setting_value) VALUES
