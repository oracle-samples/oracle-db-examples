...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('part_clas_sample');
...
BEGIN
  ...
  v_setlst(dbms_data_mining.odms_partition_columns) := 'CUST_GENDER';
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE part_clas_settings';
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
BEGIN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE part_clas_settings (setting_name VARCHAR2(30), setting_value VARCHAR2(4000))';
END;
/

BEGIN
INSERT INTO part_clas_settings (setting_name, setting_value) VALUES
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL('part2_clas_sample');
  EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
  ...
  v_setlst(dbms_data_mining.prep_auto) := dbms_data_mining.prep_auto_on;
  v_setlst(dbms_data_mining.svms_kernel_function) := dbms_data_mining.svms_linear;
  v_setlst(dbms_data_mining.odms_partition_columns) := 'CUST_GENDER,CUST_INCOME_LEVEL';
  DBMS_DATA_MINING.CREATE_MODEL2(
...
INSERT INTO part_clas_settings (setting_name, setting_value) VALUES
    ...
    (dbms_data_mining.svms_kernel_function, dbms_data_mining.svms_linear);
    INSERT INTO part_clas_settings (setting_name, setting_value) VALUES
    (dbms_data_mining.odms_partition_columns, 'CUST_GENDER,CUST_INCOME_LEVEL');
END;
/
DECLARE
  xform_list dbms_data_mining_transform.TRANSFORM_LIST;
...
-- BEGIN
--   EXECUTE IMMEDIATE 'DROP VIEW EXT_MINING_DATA_BUILD_V';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /

-- BEGIN
--   EXECUTE IMMEDIATE 'DROP VIEW EXT_MINING_DATA_APPLY_V';
--   EXCEPTION WHEN OTHERS THEN NULL;
-- END;
-- /
