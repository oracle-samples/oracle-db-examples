...
buildAllClassificationModels <- function(INPUT_DATA,TEST_DATA, formula_class, 
  ...
  #  if (grepl(feedback, "FULL|TRUE", fixed = TRUE)) cat('Spark MLlib Lasso...')
  #  timing_lasso <- list()
  #  start.time <- Sys.time()
  #  model_lasso <- orch.ml.lasso(formula = formula_class, data = INPUT_DATA, regParam = 0.3,
  #                               standardization = TRUE, convergenceTol = 1e-04, 
  #                               maxIterations = 200L, verbose=verbose_user)
  #  build_timing <- Sys.time()-start.time
  #  timing_lasso[[1]] <- as.numeric(build_timing,units = "secs")
  #  timing_lasso[[2]] <- format(build_timing,digits=4)
  #  start.time <- Sys.time()
  #  pred_lasso <- predict(model_lasso, newdata = TEST_DATA, supplemental = targetFromFormula, verbose=verbose_user)
  #  score_timing <- Sys.time()-start.time
  #  timing_lasso[[3]] <- as.numeric(score_timing,units = "secs")
  #  timing_lasso[[4]] <- format(score_timing,digits=4)
  #  if (grepl(feedback, "FULL|TRUE", fixed = TRUE)) cat('done in ',timing_lasso[[1]]+timing_lasso[[3]] ,'secs \n')
